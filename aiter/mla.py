...
def mla_decode_fwd_opus(
    q_buffer,  # [B, H, 576]         fp8 (merged nope+rope)
    kv_buffer,  # [total_tokens, 576] fp8 (merged nope+rope)
    o,  # [B, H, 512] bf16 (allocated if None)
    qo_indptr,
    kv_indptr,
    kv_indices,
    kv_last_page_lens,
    ...
    # aiter persistent metadata (generated by the caller) -- reused as-is.
    num_kv_splits=None,
    work_meta_data=None,  # accepted for API parity; unused by stage1
    work_indptr=None,
    work_info_set=None,
    reduce_indptr=None,
    ...
    q_scale=None,  # float[1] per-tensor descale (s_descale_q)
    kv_scale=None,  # float[1] per-tensor descale (s_descale_k)
    return_lse=False,
):
    """Opus merged-buffer MLA decode fp8 (mla_decode_fwd_16mx8_32nx1_fp8fp8_ps_opus), gfx950 only.

    Same persistent-scheduling pipeline as mla_decode_fwd_ds32 (reuses aiter
    metadata work_indptr/work_info_set + mla_reduce_v1), but q/kv are a single
    merged d=576 fp8 buffer (nope [0,512) + rope [512,576)) and q_scale/kv_scale
    are per-tensor scalar float descales rather than per-block E8M0 uint8.
    """
    assert get_gfx() == "gfx950", "mla_decode_fwd_opus is only supported on gfx950"
    device = q_buffer.device
    B, H, d_head = q_buffer.shape
    d_nope = 512
